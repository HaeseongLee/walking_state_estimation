...
% gt = readGT(filename);
[y_true, est, ~, step_phase, valid_step_num] = predict(filename);
...
yyaxis right
plot(time,P_plot(walking_start:end_index),LineWidth=1.5,Color='k')
ylim([-0.01,0.085])
set(gca,'YColor','k')
set(gca,'FontName','Times','FontSize',13,'TickLabelInterpreter','latex');
set(gca, 'YTick',[0.0, 0.03, 0.06],'YTickLabels',{'$Z_n$','$Z_s$','$Z_l$'})
ylabel("Foot Height", "FontName","Times",Interpreter='latex', FontSize=13)
xlim([0, max(time)])
...
yyaxis right
plot(time, F_plot(walking_start:end_index),LineWidth=1.5, color='k')
set(gca,'YColor','k')
set(gca, 'YLim', [-100,1000])
set(gca,'FontName','Times','FontSize',13,'TickLabelInterpreter','latex');
set(gca, 'YTick',[0.0, 200, 400, 600],'YTickLabels',{'$\tau_n$','$\tau_s$','$\tau_m$','$\tau_l$'})
ylabel("Disturbance", FontName='times', Interpreter='latex', fontsize=13)
legend("$f_x$",'$f_y$','$f_z$',...
...
cla reset
plot(time, ws_filtered(walking_start:end_index,1), LineWidth=1.5)
grid on
xlim([0, max(time)])
ylim([-0.1,3.1])
